...
// ... (init_app, init_opengl, reshape, handle_app_events, render_help, update_app, render_app are unchanged) ...
...
void init_app(App* app, int width, int height)
{
    ...
    init_scene(&(app->scene)); // Scene initializes wood_sprite_common_spin_angle
...
void render_app(App* app) {
    ...
    glPushMatrix();
    set_view(&(app->camera));
    render_scene(&(app->scene), &(app->camera)); // Pass camera
    glPopMatrix();
...
void destroy_app(App* app)
{
    ...
    if (app->scene.wood_texture_id != 0) { // ÚJ
        glDeleteTextures(1, &(app->scene.wood_texture_id));
    }
