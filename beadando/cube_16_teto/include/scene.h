...
typedef struct RoofParams {
    vec3 position;      // A tető alapjának középpontja (X, Y), és a gerinc magassága Z-ben
    float room_width;   // A szoba szélessége (pl. X mentén)
    float room_length;  // A szoba hossza (pl. Y mentén)
    float wall_height;  // A falak magassága, ami fölé a tető kerül
    float roof_height;  // A tetőgerinc magassága a falak tetejétől mérve
    // Anyagjellemzők a tetőhöz (opcionális, ha nem akarjuk ugyanazt, mint a falak)
    // Material material;
} RoofParams;
...
typedef struct Scene
{
    ...
    RoofParams roof; // <--- ÚJ: Tető paraméterei
...
void draw_wall(const Wall* wall, GLuint texture_id); // Új, ha csak a textúra ID-t adod át
