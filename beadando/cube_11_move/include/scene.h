...
typedef struct Scene
{
    ...
    vec3 lady_figure_position;      // Current position
    vec3 lady_figure_start_pos;     // Start point of the movement line
    vec3 lady_figure_end_pos;       // End point of the movement line
    float lady_figure_speed;        // Movement speed (units per second)
    bool lady_figure_moving_to_end; // True if moving towards end_pos, false if towards start_pos
} Scene;
...
/**
 * Update the scene.
 * Frissíti a jelenet dinamikus elemeit, pl. a figura mozgását.
 */
void update_scene(Scene* scene, double elapsed_time); // Added elapsed_time
