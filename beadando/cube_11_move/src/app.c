...
void init_app(App* app, int width, int height)
{
    ...
    init_scene(&(app->scene)); // Scene now initializes animation parameters
    app->uptime = (double)SDL_GetTicks() / 1000; // Initialize uptime
    app->is_running = true;
...
void update_app(App* app)
{
    ...
    // Pass elapsed_time to update_scene for animations
...
void render_app(App* app) {
    ...
    glPushMatrix();
    set_view(&(app->camera));
    render_scene(&(app->scene)); // Scene now renders the figure at its animated position
    glPopMatrix();
