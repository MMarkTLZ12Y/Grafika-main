...
int main(int argc, char* argv[]) {
    ...
    while (running) {
        ...
        int seconds_passed = (current_time - start_time) / 1000;
        int time_left = TIME_LIMIT - seconds_passed;

        if (time_left != last_logged_second && time_left >= 0) {
            printf("Hátralévő idő: %d másodperc\n", time_left);
            last_logged_second = time_left;
        }
        ...
        if (win) {
            SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, "Gratulálok!", "Nyertél!", window);
            running = false;
        }
        ...
        if (!win && time_left <= 0) {
            ...
            SDL_RenderClear(renderer);
            SDL_RenderPresent(renderer);
            SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Idő lejárt!", "Vesztettél!", window);
            running = false;
        }
